...
namespace ChickenDist.Forms
{
	public class FrmSale : Form
	{
		...
		private void InitUI()
		{
			...
			{
				Text = "📷 باركود / كود الصنف (الاسكنر):",
				AutoSize = true,
				Font = new Font("Segoe UI", 9.5f, FontStyle.Bold),
				ForeColor = Theme.TextMain,
				Margin = new Padding(3, 6, 2, 0)
			};
		...
		private void BarcodeTimer_Tick(object sender, EventArgs e)
		{
			...
			if (foundItem != null)
			{
				...
				{
					if (foundItem.DefaultSaleUnit == "الصغرى" && !string.IsNullOrEmpty(foundItem.Unit1Name))
					{
						matchedUnit = 1;
					}
					else if (foundItem.DefaultSaleUnit == "الوسطى" && !string.IsNullOrEmpty(foundItem.Unit2Name))
					{
						matchedUnit = 2;
					}
				}
		...
		private void DgItems_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			else if (dgItems.Columns[e.ColumnIndex].Name == "UnitPrice")
			{
				...
				if (decimal.TryParse(dataGridViewRow.Cells["UnitPrice"].Value?.ToString(), out var result2) && result2 > 0m)
				{
					...
					if (!Session.CanSellBelowCost("Sales") && saleItemDTO.PurchasePrice > 0m && result2 < saleItemDTO.PurchasePrice)
					{
					...
					if (!Session.CanSellBelowCost("Sales") && saleItemDTO.PurchasePrice > 0m && testNetUnit < saleItemDTO.PurchasePrice)
					{
						string costNotice = Session.CanViewCost("Sales") ? $" أقل من سعر التكلفة ({saleItemDTO.PurchasePrice:N2})." : " أقل من الحد الأدنى المسموح به للبيع.";
			...
			else if (dgItems.Columns[e.ColumnIndex].Name == "DiscountPct")
			{
				if (decimal.TryParse(dataGridViewRow.Cells[e.ColumnIndex].Value?.ToString(), out var resultPct) && resultPct >= 0m && resultPct <= 100m)
				{
					...
					if (!Session.CanSellBelowCost("Sales") && saleItemDTO.PurchasePrice > 0m && netUnitPrice < saleItemDTO.PurchasePrice)
					{
						string costNotice = Session.CanViewCost("Sales") ? $" أقل من سعر التكلفة ({saleItemDTO.PurchasePrice:N2})." : " أقل من الحد الأدنى المسموح به للبيع.";
		...
		/// <summary>حساب تكلفة الشراء الحقيقية للوحدة المحددة بدقة بالقسمة على المعامل إذا لم يكن مسجلاً سعر شراء صريح للوحدة</summary>
		private decimal GetUnitCost(ComboItem product, string unitName)
		{
			if (product == null) return 0m;
			decimal majorCost = product.PurchasePrice;
			if (majorCost <= 0) return 0m;
			...
			if (!string.IsNullOrEmpty(product.Unit1Name) && string.Equals(unitName, product.Unit1Name, StringComparison.OrdinalIgnoreCase))
			{
				if (product.Unit1PurchasePrice > 0) return product.Unit1PurchasePrice;
				return totalFactor > 0 ? Math.Round(majorCost / totalFactor, 2) : majorCost;
			}
			if (!string.IsNullOrEmpty(product.Unit2Name) && string.Equals(unitName, product.Unit2Name, StringComparison.OrdinalIgnoreCase))
			{
				if (product.Unit2PurchasePrice > 0) return product.Unit2PurchasePrice;
				return u3f > 0 ? Math.Round(majorCost / u3f, 2) : (totalFactor > 0 ? Math.Round(majorCost / totalFactor * u2f, 2) : majorCost);
			}
			return majorCost;
		}
		...
		private void HandleUnitChange(DataGridViewRow row, SaleItemDTO dto, string newUnit)
		{
			...
			decimal unitCost = GetUnitCost(prod, newUnit);
		...
		private void AddOrUpdateProduct(int productID, decimal qtyToAdd, decimal? manualPrice = null, bool deferRefresh = false, string unitName = null, string scannedBarcode = null, decimal discountPct = 0m, decimal discountAmt = 0m)
		{
			...
			// إذا لم يتم تحديد الوحدة حتى الآن، نعتمد الوحدة الافتراضية المحددة في كارت الصنف
			if (string.IsNullOrEmpty(unitName))
			{
				string defUnit = product.DefaultSaleUnit;
				if (defUnit == "الصغرى" && !string.IsNullOrEmpty(product.Unit1Name))
				{
					unitName = product.Unit1Name;
					if (!manualPrice.HasValue && product.Unit1SalePrice > 0)
						manualPrice = product.Unit1SalePrice;
				}
				else if (defUnit == "الوسطى" && !string.IsNullOrEmpty(product.Unit2Name))
				{
					unitName = product.Unit2Name;
					if (!manualPrice.HasValue && product.Unit2SalePrice > 0)
						manualPrice = product.Unit2SalePrice;
			...
			decimal unitCost = GetUnitCost(product, unitName);
			if (!Session.CanSellBelowCost("Sales") && manualPrice.HasValue && unitCost > 0m && manualPrice.Value < unitCost)
			{
				string costNotice = Session.CanViewCost("Sales") ? $" أقل من سعر التكلفة ({unitCost:N2}) للصنف '{product.Name}'." : $" أقل من الحد الأدنى المسموح به للصنف '{product.Name}'.";
			...
			decimal defaultUnitPrice = (string.Equals(unitName, product.Unit1Name, StringComparison.OrdinalIgnoreCase) && product.Unit1SalePrice > 0)
				? product.Unit1SalePrice
				: ((string.Equals(unitName, product.Unit2Name, StringComparison.OrdinalIgnoreCase) && product.Unit2SalePrice > 0)
					? product.Unit2SalePrice
					: product.Price);
		...
		private SaleItemDTO CreateSaleItemDTO(ComboItem product, decimal qty, decimal price, decimal stock, string unitName = null, int? batchID = null, DateTime? expiryDate = null, decimal discountPct = 0m, decimal discountAmt = 0m)
		{
			...
			if (string.IsNullOrEmpty(selectedUnit))
			{
				...
				{
					selectedUnit = !string.IsNullOrEmpty(product.BaseUnitName) ? product.BaseUnitName : (!string.IsNullOrEmpty(product.Unit1Name) ? product.Unit1Name : "وحدة");
				}
			}

			decimal purchasePrice = GetUnitCost(product, selectedUnit);
			...
			if (!string.IsNullOrEmpty(selectedUnit))
			{
				if (!string.IsNullOrEmpty(product.Unit2Name) && selectedUnit == product.Unit2Name)
				{
					...
					{
						if (product.Unit2SalePrice > 0) price = product.Unit2SalePrice;
					}
				}
				else if (!string.IsNullOrEmpty(product.Unit1Name) && selectedUnit == product.Unit1Name)
				{
					...
					if (price <= 0 || price == product.Price)
					{
						if (product.Unit1SalePrice > 0) price = product.Unit1SalePrice;
					}
				}
		...
		private void SaveInvoiceLogic(bool isDraft)
		{
			...
			if (!Session.CanSellBelowCost("Sales"))
			{
		...
		private void BtnPreview_Click(object sender, EventArgs e)
		{
			...
			itemPrep.Click += (s2, e2) => PrintPreparationSlip(showPreview: true);
		...
		public void PrintPreparationSlip(string paperSize = null, bool showPreview = false)
		{
			...
			pd.PrintPage += (s, e) =>
			{
				...
				if (itemIdx == 0)
				{
					if (isDisbursement)
					{
						// نموذج إذن صرف بضاعة (مطابق تماماً للصورة المرفقة)
						SizeF szComp = g.MeasureString(companyName, fontCompany);
						g.DrawString(companyName, fontCompany, Brushes.Black, left + (width - szComp.Width) / 2, y);
						...
						if (!isReceipt)
						{
							...
							string refLine = $"المرجع / الفاتورة: {invoiceCode} ({saleTypeStr})";
							g.DrawString(refLine, fontBody, Brushes.Gray, right - g.MeasureString(refLine, fontBody).Width, y);
							g.DrawString($"المخزن: {whName} | المسؤول: {empName}", fontBody, Brushes.Black, left, y);
							y += (isA4Page ? 22 : 18);
						}
						else
						{
					...
					else if (isModern)
					{
						...
						if (!isReceipt)
						{
							g.DrawString($"المخزن: {whName}", fontHeader, Brushes.Black, right - g.MeasureString($"المخزن: {whName}", fontHeader).Width, y);
							...
							g.DrawString($"المرجع: {invoiceCode}", fontBody, Brushes.Black, left, y);
							y += (isA4Page ? 22 : 18);
						}
						else
						{
				...
				int headerRowH = isReceipt ? 22 : (isA4Page ? 32 : 26);
				...
				{
					// توزيع أعمدة النموذج القياسي (# | الكود | اسم الصنف | الكمية المطلوبة | الوحدة | مكان التخزين / الرف)
				...
				// ── 3. سطور أصناف التحضير بارتفاع ديناميكي لاسم الصنف ──
				while (itemIdx < _items.Count)
				{
					...
					int baseH = isReceipt ? 22 : (isA4Page ? 30 : 26);
					...
					if (isDisbursement || isModern)
					{
						if (!isReceipt)
						{
							...
							curX -= colProdW;  g.DrawRectangle(penGrid, curX, y, colProdW, actualRowH);  g.DrawString(item.ProductName, fontBody, Brushes.Black, new RectangleF(curX + 4, y + 1, colProdW - 8, actualRowH - 2), sfProd);
							...
							curX -= colQtyW;   g.DrawRectangle(penGrid, curX, y, colQtyW, actualRowH);   g.DrawString(qty, fontBold, Brushes.Black, new RectangleF(curX, y, colQtyW, actualRowH), sfCenter);
						...
						{
							int tx = y;
							g.DrawString(code, fontBody, Brushes.Gray, right - colCodeW, tx);
							g.DrawString(item.ProductName, fontBody, Brushes.Black, new RectangleF(right - colCodeW - colProdW, tx, colProdW, actualRowH), sfProd);
							g.DrawString(qty, fontBold, Brushes.Black, right - colCodeW - colProdW - colQtyW, tx);
							g.DrawString(unit, fontBody, brushDarkBlue, right - colCodeW - colProdW - colQtyW - colUnitW, tx);
					...
					else
					{
						...
						else
						{
							...
							g.DrawString(loc,              fontBold,   brushDarkBlue,  right - colNumW - colProdW - colQtyW - colUnitW - colExtraW,tx);
					...
					int footerReserve = isReceipt ? 40 : (isA5 ? 70 : 90);
					if (y + actualRowH + footerReserve > pageHeight && itemIdx < _items.Count)
					{
						e.HasMorePages = true;
						return;
					}
        ...
        {
            if (!Session.PromptAdminPassword(this, "لتخصيص وترتيب أعمدة جدول المبيعات"))
                return;
