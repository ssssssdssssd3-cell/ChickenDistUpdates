    ...
    public static class SaleDAL
    {
            ...
            int? visaAccountID = null, decimal? visaPaid = null, string customClientName = null)
        {
            ...
            DbHelper.RunInTransaction((con, trans) =>
            {
                ...
                {
                    decimal costToSave = 0m;
                    if (item.PurchasePrice > 0m)
                    {
                        costToSave = item.Factor > 0m ? (item.PurchasePrice / item.Factor) : item.PurchasePrice;
                    }
                    else
                    {
                    ...
                    if (!isDraft)
                    {
                        ...
                        {
                            AppLogger.Warn($"Failed to update PendingQtyThreshold for ProductID {item.ProductID}: {exPending.Message}", "SalesDAL");
                        }
                    }
                }
