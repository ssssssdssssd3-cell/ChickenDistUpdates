        ...
        private static BOMModel MapBOM(DataRow row)
        {
            ...
            var dtItems = DbHelper.Query(@"
                SELECT bi.*, p.ProductCode AS RawProductCode, p.ProductName AS RawProductName, 
                       COALESCE(NULLIF(p.CostPrice, 0), NULLIF(p.PurchasePrice, 0), (SELECT TOP 1 pi2.UnitPrice FROM PurchaseItems pi2 WHERE pi2.ProductID = p.ProductID ORDER BY pi2.ItemID DESC), 0) AS RawCostPrice
                FROM BOMItems bi
