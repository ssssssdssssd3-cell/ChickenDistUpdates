    ...
    public class FrmPOS : Form
    {
        ...
        private void InitUI()
        {
            ...
            dgItems = new DataGridView
            {
                ...
                BackgroundColor = Theme.BgCard, ForeColor = Theme.TextMain,
                ...
                DefaultCellStyle = new DataGridViewCellStyle { BackColor = Theme.BgCard, ForeColor = Theme.TextMain, SelectionBackColor = Theme.Accent, SelectionForeColor = Color.White },
                GridColor = Theme.BorderColor, BorderStyle = BorderStyle.None, CellBorderStyle = DataGridViewCellBorderStyle.SingleHorizontal
            };
            ...
            // ── لوحة العميل ───────────────────────────────────
            var pnlClient = new Panel { Location = new Point(660, 85), Size = new Size(420, 55), BackColor = Theme.BgCard };
            ...
            pnlQuick = new Panel { Location = new Point(660, 150), Size = new Size(420, 335), BackColor = Theme.BgCard };
            pnlQuick.Paint += (s, e) => Theme.DrawCardBorder(e.Graphics, pnlQuick);
            var lQuick = new Label { Text = "⚡ أصناف سريعة", Location = new Point(5, 5), Size = new Size(410, 25), ForeColor = Theme.Accent, Font = new Font("Segoe UI", 10f, FontStyle.Bold), TextAlign = ContentAlignment.MiddleCenter };
            flowQuickItems = new FlowLayoutPanel { Location = new Point(5, 35), Size = new Size(410, 295), AutoScroll = true, BackColor = Color.Transparent };
            ...
            lblItemCount = new Label { Text = "عدد الأصناف: 0", Location = new Point(20, 10), Size = new Size(250, 30), ForeColor = Theme.TextSub, Font = new Font("Segoe UI", 11f) };
            lblTotal = new Label { Text = "الإجمالي: 0.00 ج", Location = new Point(20, 45), Size = new Size(350, 40), ForeColor = Theme.Success, Font = new Font("Segoe UI", 20f, FontStyle.Bold) };

            var lPaid = new Label { Text = "المدفوع:", Location = new Point(400, 45), Size = new Size(80, 30), ForeColor = Theme.TextMain, Font = new Font("Segoe UI", 12f) };
            txtPaid = new TextBox { Location = new Point(490, 42), Size = new Size(160, 35), Font = new Font("Segoe UI", 16f, FontStyle.Bold), BackColor = Theme.BgInput, ForeColor = Color.Black, BorderStyle = BorderStyle.FixedSingle, Text = "0", TextAlign = HorizontalAlignment.Center };
            txtPaid.TextChanged += (s, e) => RecalcChange();

            lblChange = new Label { Text = "الباقي: 0.00 ج", Location = new Point(660, 45), Size = new Size(350, 40), ForeColor = Theme.Accent, Font = new Font("Segoe UI", 20f, FontStyle.Bold) };
            ...
            btnNew = Theme.MakeButton("🔄 فاتورة جديدة (F2)", Color.FromArgb(60, 70, 85), new Point(280, 130), new Size(220, 55));
            btnNew.Font = new Font("Segoe UI", 12f, FontStyle.Bold);
            btnNew.Click += (s, e) => NewInvoice();

            btnCancel = Theme.MakeButton("❌ إلغاء (Esc)", Theme.Danger, new Point(510, 130), new Size(180, 55));
            ...
            var btnPrint = Theme.MakeButton("🖨️ طباعة آخر (F6)", Theme.Primary, new Point(700, 130), new Size(180, 55));
            btnPrint.Font = new Font("Segoe UI", 12f, FontStyle.Bold);
            btnPrint.Click += (s, e) => { if (_lastSaleID > 0) PrintReceipt(_lastSaleID); };

            pnlTotals.Controls.Add(lblItemCount);
            pnlTotals.Controls.Add(lblTotal);
            pnlTotals.Controls.Add(lPaid);
            ...
            pnlTotals.Controls.Add(btnPrint);
        ...
        private void LayoutPanels()
        {
            ...
            var pnlClient = this.Controls[3] as Panel;
        ...
        private void AddProductByCode(string code)
        {
            ...
            if (dt.Rows.Count == 0)
            {
                ...
                if (code.Length >= 8 && code.StartsWith(AppConfig.BarcodeScalePrefix))
                {
                    ...
                    dt = DbHelper.Query("SELECT p.ProductID, p.ProductCode, p.ProductName, p.Unit, p.SalePrice, p.PurchasePrice, p.Unit1Name, p.Unit1Barcode, p.Unit1SalePrice, p.Unit2Name, p.Unit2Barcode, p.Unit2SalePrice, p.Unit2Factor FROM Products p WHERE p.IsActive = 1 AND p.ProductCode = @c", DbHelper.P("@c", itemCode));
                    if (dt.Rows.Count > 0 && weight > 0)
                    {
                        AddItemFromRow(dt.Rows[0], weight, null, 1m);
                        return;
                    }
                }
            ...
            AddItemFromRow(row, 1, unitName, factor, price);
        }

        private void AddItemFromRow(DataRow row, decimal qty, string unitName, decimal factor, decimal overridePrice = 0)
        {
                ...
                DbHelper.RunInTransaction((con, trans) =>
                {
                    ...
                    foreach (var item in _items)
                    {
                        DbHelper.ExecuteInsertTrans(trans,
                            @"INSERT INTO SaleItems (SaleID,ProductID,Quantity,UnitPrice,TotalPrice,DiscountPct,DiscountAmt,PriceTier,UnitName,Factor)
                              VALUES (@sid,@pid,@qty,@up,@tp,0,0,N'قطاعي',@un,@f)",
                        ...
                        DbHelper.ExecuteTrans(trans,
                            ...
                            DbHelper.P("@pid", item.ProductID), DbHelper.P("@wid", warehouseID), DbHelper.P("@q", baseQty));
                    }
        ...
        private void OpenProductSearch()
        {
            try
            {
                ...
                {
                    var dt = DbHelper.Query("SELECT p.ProductID, p.ProductCode, p.ProductName, p.Unit, p.SalePrice, p.PurchasePrice, p.Unit1Name, p.Unit1Barcode, p.Unit1SalePrice, p.Unit2Name, p.Unit2Barcode, p.Unit2SalePrice, p.Unit2Factor FROM Products p WHERE p.ProductID = @id", DbHelper.P("@id", frm.SelectedProductID));
                    if (dt.Rows.Count > 0) AddItemFromRow(dt.Rows[0], 1, null, 1m);
                }
            }
            catch { }
        }
        ...
        private void LoadQuickItems()
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                btn.Click += (s, e) =>
                {
                    var dtP = DbHelper.Query("SELECT p.ProductID, p.ProductCode, p.ProductName, p.Unit, p.SalePrice, p.PurchasePrice, p.Unit1Name, p.Unit1Barcode, p.Unit1SalePrice, p.Unit2Name, p.Unit2Barcode, p.Unit2SalePrice, p.Unit2Factor FROM Products p WHERE p.ProductID=@id", DbHelper.P("@id", (int)((Button)s).Tag));
